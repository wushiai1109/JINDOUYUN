...
import com.jindouyun.wx.service.*;
...
public class WxCartController {
    ...
    @Autowired
    private JindouyunCouponVerifyService couponVerifyService;
    ...
    public Object index(@LoginUser Integer userId) {
        ...
        List<JindouyunCart> list = cartService.queryByUid(userId);
        List<JindouyunCart> cartList = new ArrayList<>();
        // 如果系统检查商品已删除或已下架，则系统自动删除。
        // 更好的效果应该是告知用户商品失效，允许用户点击按钮来清除失效商品。
        for (JindouyunCart cart : list) {
        ...
        for (JindouyunCart cart : cartList) {
     ...
     */
    @PostMapping("add")
    public Object add(@LoginUser Integer userId, @RequestBody JindouyunCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (cart == null) {
        ...
        //判断商品是否可以购买
        JindouyunGoods goods = goodsService.findById(goodsId);
        if (goods == null || !goods.getIsOnSale()) {
        ...
        //判断购物车中是否存在此规格商品
        JindouyunCart existCart = cartService.queryExist(goodsId, productId, userId);
        if (existCart == null) {
            //取得规格的信息,判断规格库存
     ...
     */
    @PostMapping("fastadd")
    public Object fastadd(@LoginUser Integer userId, @RequestBody JindouyunCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (cart == null) {
     ...
     */
    @PostMapping("update")
    public Object update(@LoginUser Integer userId, @RequestBody JindouyunCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (cart == null) {
        ...
        //判断是否存在该订单
        // 如果不存在，直接返回错误
        JindouyunCart existCart = cartService.findById(id);
        if (existCart == null) {
    ...
    public Object goodscount(@LoginUser Integer userId) {
        ...
        int goodsCount = 0;
        List<JindouyunCart> cartList = cartService.queryByUid(userId);
        for (JindouyunCart cart : cartList) {
            goodsCount += cart.getNumber();
        }
    ...
    public Object checkout(@LoginUser Integer userId, Integer cartId, Integer addressId, Integer couponId, Integer userCouponId, Integer grouponRulesId) {
        ...
        // 收货地址
        JindouyunAddress checkedAddress = null;
        if (addressId == null || addressId.equals(0)) {
            checkedAddress = addressService.findDefault(userId);
            // 如果仍然没有地址，则是没有收货地址
            // 返回一个空的地址id=0，这样前端则会提醒添加地址
            if (checkedAddress == null) {
                checkedAddress = new JindouyunAddress();
