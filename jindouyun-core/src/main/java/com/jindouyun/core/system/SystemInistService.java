...
class SystemInistService {
    ...
    static {
        // 小程序相关配置默认值
        DEFAULT_CONFIGS.put(SystemConfig.Jindouyun_WX_INDEX_NEW, "6");
        DEFAULT_CONFIGS.put(SystemConfig.Jindouyun_WX_INDEX_HOT, "6");
        DEFAULT_CONFIGS.put(SystemConfig.Jindouyun_WX_INDEX_BRAND, "4");
        DEFAULT_CONFIGS.put(SystemConfig.Jindouyun_WX_INDEX_TOPIC, "4");
        DEFAULT_CONFIGS.put(SystemConfig.Jindouyun_WX_INDEX_CATLOG_LIST, "4");
        ...
        DEFAULT_CONFIGS.put(SystemConfig.Jindouyun_ORDER_COMMENT, "7");
        // 商城相关配置默认值
        DEFAULT_CONFIGS.put(SystemConfig.Jindouyun_MALL_NAME, "Jindouyun");
        DEFAULT_CONFIGS.put(SystemConfig.Jindouyun_MALL_ADDRESS, "上海");
        DEFAULT_CONFIGS.put(SystemConfig.Jindouyun_MALL_Latitude, "31.201900");
    ...
    @Autowired
    private JindouyunSystemConfigService JindouyunSystemConfigService;

    private void initConfigs() {
        // 1. 读取数据库全部配置信息
        Map<String, String> configs = JindouyunSystemConfigService.queryAll();
        ...
        for (Map.Entry<String, String> entry : DEFAULT_CONFIGS.entrySet()) {
            ...
            JindouyunSystemConfigService.addConfig(entry.getKey(), entry.getValue());
        }
    ...
    private Map<String, String> getSystemInfo() {
        ...
        infos.put("调试级别", environment.getProperty("logging.level.org.linlinjava.Jindouyun.wx"));

        // 测试获取application-core.yml配置信息
        infos.put(SystemInfoPrinter.CREATE_PART_COPPER + 1, "模块状态");
        infos.put("邮件", environment.getProperty("Jindouyun.notify.mail.enable"));
        infos.put("短信", environment.getProperty("Jindouyun.notify.sms.enable"));
        infos.put("模版消息", environment.getProperty("Jindouyun.notify.wx.enable"));
        infos.put("快递信息", environment.getProperty("Jindouyun.express.enable"));
        infos.put("快递鸟ID", environment.getProperty("Jindouyun.express.appId"));
        infos.put("对象存储", environment.getProperty("Jindouyun.storage.active"));
        infos.put("本地对象存储路径", environment.getProperty("Jindouyun.storage.local.storagePath"));
        infos.put("本地对象访问地址", environment.getProperty("Jindouyun.storage.local.address"));
        infos.put("本地对象访问端口", environment.getProperty("Jindouyun.storage.local.port"));

        // 微信相关信息
        infos.put(SystemInfoPrinter.CREATE_PART_COPPER + 2, "微信相关");
        infos.put("微信APP KEY", environment.getProperty("Jindouyun.wx.app-id"));
        infos.put("微信APP-SECRET", environment.getProperty("Jindouyun.wx.app-secret"));
        infos.put("微信支付MCH-ID", environment.getProperty("Jindouyun.wx.mch-id"));
        infos.put("微信支付MCH-KEY", environment.getProperty("Jindouyun.wx.mch-key"));
        infos.put("微信支付通知地址", environment.getProperty("Jindouyun.wx.notify-url"));
